...
def get_text_image(shape, text, color=(1, 1, 1), border=0.2, font=None):
    h, w, c = shape
    try:
        pil_font = _get_pil_font(get_default_ttf_font_name(), h - 2)
...
def get_default_ttf_font_name():
    ...
    if platform[0:3] == "win":
        return windows_font_name_map.get("en", "cour")
    elif platform == "darwin":
        return darwin_font_name_map.get("en", "cour")
    else:
        return linux_font_name_map.get("en", "cour")
