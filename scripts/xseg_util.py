...
from cv2 import cv2
from lib.deepfacelab import pathex
from lib.image_alt import read_image, DflPng
from lib.deepfacelab.interact import interact as io
from lib.deepfacelab.leras.neuralnet import NeuralNet
from lib.deepfacelab import LandmarksProcessor
from lib.deepfacelab.FaceType import FaceType
from lib.xseg_net import XSegNet
import pickle
...
def apply_xseg(input_path, model_path):
    ...
    if face_type is None:
        face_type = io.input_str("XSeg model face type", 'same', ['h', 'mf', 'f', 'wf', 'head', 'same'],
                                 help_message="Specify face type of trained XSeg model. For example if XSeg model"
                                              "trained as WF, but faceset is HEAD, specify WF to apply xseg only on WF"
                                              "part of HEAD. Default is 'same'").lower()
        if face_type == 'same':
            face_type = None
    ...
    device_config = NeuralNet.DeviceConfig.ask_choose_device(choose_only_one=True)
    NeuralNet.initialize(device_config)

    xseg = XSegNet(name='XSeg',
                   load_weights=True,
                   weights_file_root=model_path,
                   data_format=NeuralNet.data_format,
                   raise_on_no_model_files=True)
    xseg_res: int = xseg.get_resolution()
    print("XSeg resolution determined to be:", str(xseg_res))

    images_paths = pathex.get_image_paths(input_path, return_Path_class=True)

    for filepath in io.progress_bar_generator(images_paths, "Processing"):
        dflimg = DflPng.load(filepath)
        if dflimg is None or not dflimg.has_metadata:
            io.log_info(f'{filepath} is not a DFLIMG')
            continue

        img = read_image(filepath).astype(np.float32) / 255.0
        # img = dflimg.face_centered_image.astype(np.float32) / 255.
        h, w, c = img.shape

        img_face_type = FaceType.fromString(dflimg.face_type)
        if face_type is not None and img_face_type != face_type:
            lmrks = dflimg.source_landmarks
